...
(function (root, factory) {
	...
	chartjs.Type.extend({
		...
		update: function (ease, mousePosition) {
			...
			if (this._hasData()) {
				...
				if (mousePosition && mousePosition.x != 0) {
					...
					ctx.lineTo(mousePosition.x, this.scale.calculateY(this.scale.yScaleRange.min) + 4);
					ctx.strokeStyle = this.options.axisStrokeColor;
					ctx.lineWidth = this.options.axisStrokeWidth;
					ctx.stroke();
					ctx.closePath();
				}
