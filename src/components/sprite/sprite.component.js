...
export default class Sprite extends React.Component {
  ...
  render() {
    ...
    for (let i = 1; i < this.props.item.animations.length && !found; i++) {
      if (frame >= prev.frame && frame <= this.props.item.animations[i].frame) {
        ...
        if (this.state.wrapper) {
          if (i == 1) {
            pos1 = [prev.position[0] * this.props.layout.width, prev.position[1] * this.props.layout.height - this.state.wrapper.clientHeight * 0.5, prev.position[2]];
            pos2 = [current.position[0] * this.props.layout.width, current.position[1] * this.props.layout.height - this.state.wrapper.clientHeight * 0.5, current.position[2]];
          } else {
    ...
    if (!found && frame <= this.props.item.animations[0].frame) {
      ...
      if (this.state.wrapper) {
        position = [temp.position[0] * this.props.layout.width, temp.position[1] * this.props.layout.height - this.state.wrapper.clientHeight * 0.5, temp.position[2]];
      } else {
      ...
      if (this.state.wrapper) {
        position = [temp.position[0] * this.props.layout.width - this.state.wrapper.clientWidth, temp.position[1] * this.props.layout.height - this.state.wrapper.clientHeight * 0.5, temp.position[2]];
      } else {
        position = [temp.position[0] * this.props.layout.width, temp.position[1] * this.props.layout.height, temp.position[2]];
      }
    }
