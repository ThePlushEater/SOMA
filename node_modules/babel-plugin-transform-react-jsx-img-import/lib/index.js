...
function hasImgArgument(path) {
  return (path.get('arguments').length > 1 && path.get('arguments')[0].isStringLiteral({ value: 'img' })) || (path.get('arguments').length > 1 && path.get('arguments')[0].isStringLiteral({ value: 'image' }));
}
...
function getImgSrcNodePath(path) {
  ...
  if (path.get('arguments')[1].isObjectExpression()) {
    ...
    for (let prop of props) {
      ...
      if (key.isIdentifier({ name: 'src' }) || key.isIdentifier({ name: 'href' })) {
